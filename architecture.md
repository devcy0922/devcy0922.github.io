...
sequenceDiagram
    ...
    Gateway-->>Browser: 최종 답변 Token Stream
...
- 모델의 `reasoning_content`는 Gateway 연결을 유지하는 SSE 이벤트로 전달될 수 있지만 공개 화면에는 원문을 표시하지 않는다.
- 브라우저는 `reasoning_content`를 진행 상태로만 반영하고, 검증 가능한 최종 `content`만 실시간으로 표시한다.
- 결과 패널은 입력 패널 옆에 고정해 응답이 추가되더라도 페이지 전체 높이가 늘어나지 않게 한다.
- 모바일에서는 첫 스트림 수신 시 결과 패널을 자동으로 화면 안에 배치한다.
