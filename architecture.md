...
sequenceDiagram
    ...
    Gateway-->>Browser: Completion + Trace ID
...
| RPM | 4 | 단일 브라우저의 자원 독점 제한 |
| 입력 길이 | 2,000자 | Prefill 비용과 DLP 표면 제한 |
| 출력 상한 | 512 tokens | 추론 시간 제한 |
...
프롬프트, 응답 원문, Authorization Header와 방문자 IP는 포트폴리오 분석용 로그에 저장하지 않는다. “실제 사람 수” 대신 개인정보를 수집하지 않는 익명 세션/요청 수로 표현한다.

## 위협과 대응

| 위협 | 대응 |
|---|---|
| 공개 Demo Key 재사용 | 전용 Project, 낮은 RPM, 단일 Model, Token 상한 적용 |
...
| 비용·자원 고갈 | RPM과 요청 크기 상한, 운영 시 Key 즉시 폐기 가능 |
...
- SurrealDB에서 Demo Key가 `portfolio-demo`, `demo`, RPM 4, `auto`로 제한된다.
