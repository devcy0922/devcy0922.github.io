...
## 📌 Status & Repository
- **상태**: `Stable`
- **역할**: 테스트에서 LLM 오류 응답과 지연을 재현하는 보조 서버
- **라이선스**: MIT
- **주요 언어**: Rust, Go
...
## 1. Problem
AI 에이전트나 Gateway 서비스를 테스트할 때 실제 외부 LLM API(OpenAI, Claude 등)를 호출하면 테스트가 Provider 상태에 종속됩니다. 응답 지연, Rate Limit과 네트워크 오류 때문에 같은 입력도 일관되게 재현하기 어렵습니다.
